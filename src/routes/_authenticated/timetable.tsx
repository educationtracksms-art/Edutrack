...
function TimetablePage() {
  ...
  const [visibleDays, setVisibleDays] = useState<string[]>(
    DAYS.map((_, index) => String(index + 1)),
  );
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase
          .from("terms")
          .select("id, name, academic_year_id, is_current, school_id")
          .eq("school_id", schoolId!),
        supabase
        ...
        supabase
          .from("academic_years")
          .select("id, name, is_current, school_id")
          .eq("school_id", schoolId!),
        supabase
  ...
  const currentYear = useMemo(() => {
    if (!data) return null;
    return (
      data.years.find((year) => year.is_current) ??
      (currentTerm
        ? (data.years.find((year) => year.id === currentTerm.academic_year_id) ?? null)
        : null)
    );
  }, [data, currentTerm]);
  ...
  const byId = {
    ...
    teacher: new Map(
      (data?.teachers ?? []).map((item) => [
        ...
        {
          name: item.full_name,
          initials: item.initials?.trim() || initialsFromName(item.full_name),
        },
      ]),
    ),
  };
  const lessonPeriods = useMemo(
    () => periodRows.filter((row) => !row.is_break && !row.is_lunch),
    [periodRows],
  );
  ...
  if (!timetableEnabled) {
    return (
      <div>
        <PageHeader
          title="Timetable"
          description="Schedule lessons, periods, and room allocations."
        />
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.8fr_1fr]">
            ...
            <div className="space-y-6">
              {visibleDayLabels.map(({ day, label }) => {
                ...
                return (
                  <table
                    key={day}
                    className="min-w-full border border-border text-[11px] sm:text-sm"
                  >
                    <thead>
                      <tr>
                        <th className="border border-border px-2 py-2 text-center font-semibold">
                          Time
                        </th>
                        <th
                        ...
                        {classStreamGroups.length ? (
                          classStreamGroups.map((group) => (
                            <th
                              key={group.id}
                              className="border border-border px-2 py-2 text-center font-semibold"
                              colSpan={Math.max(group.streams.length, 1)}
                            >
                              {group.label}
                            </th>
                          ))
                              ...
                              streamColumns.map((column) => {
                                const entry =
                                  (entriesByPeriodForDay.get(slot.period_order) ?? []).find(
                                    (item) =>
                                      item.class_id === column.classId &&
                                      item.stream_id === column.streamId,
                                  ) ?? null;
                                return (
                                  <td
                                    key={column.key}
        ...
        <div className="space-y-4">
          <Panel title="Timetable timings">
            {!canEdit ? (
              <p className="text-sm text-muted-foreground">
                Only DOS and academic managers can adjust timetable periods.
              </p>
            ) : (
              <div className="space-y-4">
                      ...
                      <div className="mb-3 flex justify-end">
                        <div className="flex gap-2">
                          <Btn
                            type="button"
                            variant="ghost"
                            onClick={() => movePeriodDraft(row.id, "up")}
                            disabled={index === 0}
                          >
                            Up
                          </Btn>
                          ...
                          <Btn
                            type="button"
                            variant="ghost"
                            onClick={() => removePeriodDraft(row.id)}
                          >
                            Remove
                          </Btn>
                        </div>
                              ...
                              setPeriodDrafts((current) =>
                                current.map((item, itemIndex) =>
                                  itemIndex === index
                                    ? {
                                        ...item,
                                        period_order: Number(e.target.value) || item.period_order,
                                      }
                                    : item,
                                ),
                              )
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setPeriodDrafts((current) =>
                                current.map((item, itemIndex) =>
                                  itemIndex === index
                                    ? { ...item, start_time: e.target.value || null }
                                    : item,
                                ),
                              )
                            }
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setPeriodDrafts((current) =>
                                current.map((item, itemIndex) =>
                                  itemIndex === index
                                    ? { ...item, end_time: e.target.value || null }
                                    : item,
                                ),
                              )
                            }
          ...
          <Panel title={form.id ? "Edit lesson" : "Create lesson"}>
            {!canEdit ? (
              <p className="text-sm text-muted-foreground">
                You can view the timetable here, but editing is reserved for DOS and academic
                managers.
              </p>
            ) : (
              <form
              ...
              >
                ...
                <Field label="Stream">
                  ...
                  >
                    ...
                    {(data?.streams ?? [])
                      .filter((stream) => !form.class_id || stream.class_id === form.class_id)
                      .map((item) => (
                        <option key={item.id} value={item.id}>
                          {item.name}
                        </option>
            ...
            {!selectedEntry ? (
              ...
              <div className="space-y-2 text-sm">
                <p className="font-semibold">
                  {byId.subjectName.get(selectedEntry.subject_id) ?? "Subject"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {teacherInitials(byId, selectedEntry.teacher_id)}
                </p>
                <p className="text-xs text-muted-foreground">
                  {DAYS[selectedEntry.day_of_week - 1]} Â· P{selectedEntry.period} Â·{" "}
...
function teacherInitials(
  byId: { teacher: Map<string, { name: string; initials: string }> },
  teacherId: string,
) {
...
function findNextSlot({
  ...
  for (let offset = 0; offset < candidates.length; offset += 1) {
    ...
    const teacherKey = JSON.stringify({
      day: candidate.day,
      period: candidate.period.period_order,
      teacher: teacherId,
    });
    const classKey = JSON.stringify({
      day: candidate.day,
      period: candidate.period.period_order,
      class_id: classId,
    });
    const streamKey = streamId
      ...
      : null;
    if (
      teacherOccupied.has(teacherKey) ||
      classOccupied.has(classKey) ||
      (streamKey ? streamOccupied.has(streamKey) : false)
    ) {
      continue;
    }
